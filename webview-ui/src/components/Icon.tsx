...
/**
 * Theme-adaptive SVG icon set for the Sqlens webview.
 *
 * Icons are drawn with `stroke="currentColor"` / `fill="currentColor"` so they
 * automatically follow the active VS Code theme (foreground, button colors,
 * high-contrast themes, etc.). No emoji and no font assets are required.
 */
...
const STROKE_ICONS: Record<string, React.ReactNode> = {
  ...
  key: (
    <>
      <path d="M12.65 10.35a5.5 5.5 0 1 1-7.78 7.78 5.5 5.5 0 0 1 7.78-7.78z" />
      <path d="M12.65 10.35 15.5 7.5l3 3L22 7l-3-3-3 3-3.35 3.35" />
    </>
  ),
  link: (
    <>
...
export default function Icon({ name, size = 14, strokeWidth = 1.7, className }: IconProps) {
  const isFilled = name in FILLED_ICONS;
  const content = isFilled ? FILLED_ICONS[name] : STROKE_ICONS[name];
  if (!content) { return null; }

  return (
    <svg
      className={`tp-icon${className ? ` ${className}` : ''}`}
      width={size}
      height={size}
      viewBox="0 0 24 24"
