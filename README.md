...
- 浏览库、表、视图、列、索引、外键。
- Schema 面板：内联筛选（`*` / `?` 通配符）、就地重命名表、完整悬浮提示，以及按驱动能力显示的右键菜单。
...
侧边栏中的 **MCP 服务器面板** 可以查看运行状态、端点与令牌，并通过两个开关控制访问：**Read & Write**（打开 = 允许写入，对应 `sqlens.mcp.readOnly` 关闭；关闭 = 只读，默认）与 **Auto-approve**（打开 = 写操作不再逐次确认，对应 `writeMode: allow`；只读模式下不可用）。面板还列出已注册的 AI 助手，并支持一键复制完整的 MCP 配置 JSON：
...
- 需要确认时，Sqlens 会主动切到 **AI 活动** 面板并弹出带 **允许 / 拒绝** 的通知，避免确认请求在后台超时被自动拒绝（`sqlens.mcp.focusOnConfirm`、`sqlens.mcp.confirmNotification`、`sqlens.mcp.autoConfirmTimeout` 可调）。
- `writeMode: allow` 时 `INSERT/UPDATE/DELETE/CREATE/ALTER` 直接执行；`DROP`、`TRUNCATE` 和破坏性管理命令在任何模式下都被拦截。
...
Sqlens 侧边栏的 **Schema** 视图列出活动连接的表与视图（目前只包含这两类对象，不含函数 / 存储过程 / 触发器 / 序列）：

- **默认展开**：表与视图分组连上连接后自动展开；你手动折叠过的分组在刷新后保持折叠，切换连接时恢复默认。
- **内联筛选**：点击标题栏的筛选图标，输入框**就地展开**（不是弹窗），边输入边过滤；支持 `*` / `?` 通配符和 `schema.table` 写法，右侧显示 `匹配 / 总数`。`Esc` 或再点一次图标即收起。
- **就地重命名**：右键表 → **重命名**，表名那一行直接变成输入框；回车提交、`Esc` 取消，名称非法时就地提示原因。
- **悬浮提示**：鼠标停留即可查看表的类型 / 行数 / 大小 / 存储引擎 / 注释，字段的类型 / 是否可空 / 默认值 / 长度 / 主键 / 外键 / 注释。
- **右键菜单**：打开表、查看结构、查看 DDL、复制 `CREATE TABLE`、生成测试数据、导出 / 导入数据、重命名、清空表、删除表；只显示当前驱动支持的操作，危险操作标红，点击别处或按 `Esc` 自动关闭。
- 视图标题栏提供 **筛选 / 新建表 / 刷新** 三个按钮。
